...
export function initOfflineSyncEngine(getState, setState, showToast) {
  ...
  const handleOnline = () => {
    ...
    syncEngineState.isOnline = true;
    notifySyncStatusChange();
    // Silent background sync - automatically push local updates without UI indicators
  ...
  const handleOffline = () => {
    ...
    syncEngineState.isOnline = false;
    notifySyncStatusChange();
    // Silent background monitoring without UI indicators
  };
...
/**
 * Creates the Sync Status Badge UI element (hidden / deprecated)
 */
export function createSyncStatusBadge() {
