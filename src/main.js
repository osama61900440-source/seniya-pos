...
export function navigateBack() {
  ...
  state.tab = "dashboard";
  try {
    window.history.pushState({ tab: "dashboard", fromSidebar: false }, "", "#dashboard");
  } catch (e) {}
  renderApp();
}
...
function setData(mutator) {
  ...
  if (typeof navigator !== "undefined" && navigator.onLine) {
    performOnlineSync(function () { return state; }, function (newData) { state.data = newData; renderApp(); }, null, false);
  } else {
...
export function renderApp() {
  ...
  if (state.tab === "items") {
    ...
    var topBarChildren = [backBtn, el("h1", { class: "inv-title", style: { flex: 1 } }, titleText)];
    ...
    app.appendChild(el("div", { class: "header" }, [mainHamBtn, el("h1", { style: { flex: 1 } }, "የሱቅ መቆጣጠሪያ ሲስተም"), el("div", { class: "header-spacer" })]));
  }
  if (!storageAvailable) {
